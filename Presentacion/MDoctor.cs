...
namespace Presentacion
{
    public partial class MDoctor : Form
    {
        ...
        private void LlenarEspecialidades()
        {
            ...
            foreach (eEspecialidad item in nEspecialidad.ListarEspecialidad())
            {
                comboBoxespecialidad.Items.Add(item);
            }
        }
        ...
        private void buttoninsertar_Click(object sender, EventArgs e)
        {
            if (textBoxapellido.Text != "" && textBoxcolegiatura.Text != "" && textBoxcontra.Text != "" && textBoxnombre.Text != "" && textBoxtelef.Text != "" && comboBoxespecialidad.SelectedIndex != -1)
            {
                ...
                especialidad =(eEspecialidad) comboBoxespecialidad.SelectedItem;
                negdoctor.InsertarDoctor(Convert.ToInt32(textBoxcolegiatura.Text), textBoxnombre.Text, textBoxapellido.Text, textBoxcontra.Text, Convert.ToInt32(textBoxtelef.Text), especialidad.idespecialidad);
                MostrarDoctores();
                LimpiarCajas();
            }
            else
        ...
        {
            // Comprueba si el carácter ingresado no es una letra
            if (!char.IsLetter(e.KeyChar) && !char.IsControl(e.KeyChar))
            {
                // Si no es una letra ni una tecla de control, cancela el evento
                e.Handled = true;
            }
        }
