...
namespace Datos
{
    public class dCita : MantenimientoDB
    {
        ...
        {
            string fechaFormateada = cita.fecha.ToString("MM-dd-yyyy");
            string insert = string.Format("insert into Cita values({0},{1},'{2}','{3}',{4})", cita.paciente.dnipaciente, cita.doctorasignado.nrocolegiatura, fechaFormateada, cita.hora, cita.diagnostico.iddiagnostico);
            return Insertar(insert);
        }
        public string actualizarCita(eCita cita)
        {
            string fechaFormateada = cita.fecha.ToString("MM-dd-yyyy");
            string update = string.Format("update Cita set dnipaciente={0},nrocolegiatura={1},fecha='{2}',hora='{3}',iddiagnostico={4} where idcita={5}", cita.paciente.dnipaciente, cita.doctorasignado.nrocolegiatura, fechaFormateada, cita.hora, cita.diagnostico.iddiagnostico, cita.idcita);
